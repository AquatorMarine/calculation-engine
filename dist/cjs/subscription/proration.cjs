...
exports.calculateAddonProration = calculateAddonProration;
exports.calculateUpgradeProration = calculateUpgradeProration;
const BILLING_PERIOD_SECONDS = {
    DAY: 1 * 24 * 60 * 60,
    WEEK: 7 * 24 * 60 * 60,
    MONTH: 30 * 24 * 60 * 60,
    YEAR: 365 * 24 * 60 * 60,
 ...
 */
function calculateAddonProration(params) {
    const now = params.now ?? Date.now() / 1000;
    const totalSeconds = Math.max(1, (params.currentPeriodEnd || now) - params.currentPeriodStart);
    const remainingSeconds = Math.max(0, (params.currentPeriodEnd || now) - now);
...
function calculateUpgradeProration(params) {
    ...
    const fallbackPeriod = BILLING_PERIOD_SECONDS[params.billingFrequency] ??
    ...
    const unusedCredit = Number((params.currentTotalCost * (remainingSeconds / totalSeconds)).toFixed(2));
    const amountDue = Math.max(0, Number((params.newTotalCost - unusedCredit).toFixed(2)));
    return {
